...
/** Enables the mise tool within the development container.
  ...
  */
private[modules] def mise(mountKey: String): Try[Module] = for {
  encodedPostCreateScript <- Modules.base64Encoded("misePostCreateCommand.sh")
  encodedOnCreateScript   <- Modules.base64Encoded("miseOnCreateCommand.sh")
} yield Module(
  name = "mise",
  summary = "Install and configure mise for dev tools management (https://mise.jdx.dev/)",
  enabledByDefault = true,
  contribution = ModuleContribution(
    onCreateCommands = List(
      Command(
        cmd = s"echo '$encodedOnCreateScript' | base64 -d | bash",
        workingDirectory = "."
      )
    ),
    postCreateCommands = List(
      Command(
        cmd = s"echo '$encodedPostCreateScript' | base64 -d | bash",
        workingDirectory = "."
      )
    ),
