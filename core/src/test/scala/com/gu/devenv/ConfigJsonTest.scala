...
class ConfigJsonTest extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks {
  "Config.configAsJson" - {
    ...
    "postStartCommand field" - {
      ...
      /** test to differentiate between (echo a) && (echo b) | sudo tee - bad ((echo a) && (echo b))
        * \| sudo tee - good
        *
        * confirm that List(Command("echo a", "."), Command("echo b", ".")) => Some("((cd . && echo
        * a) && (cd . && echo b)) | sudo tee")
        */
