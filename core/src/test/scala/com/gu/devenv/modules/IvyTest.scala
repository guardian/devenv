...
class IvyTest extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  "builtInModules" - {
    "should use the provided module config to parameterise the ivy module's mount" in {
      ...
      modules should contain(ivy(moduleConfig.mountKey))
    }
  }
}
