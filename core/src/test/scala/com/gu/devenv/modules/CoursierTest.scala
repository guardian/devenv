...
class CoursierTest extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks {
  "coursier module" - {
    "should use the provided ModuleConfig to parameterise the data mount's name" in {
      ...
      module.contribution.mounts.head match {
        case Mount.ExplicitMount(source, _, _) =>
          source shouldBe s"$mountKey-coursier-data-volume"
        case Mount.ShortMount(mount) =>
          fail(
            s"Expected an ExplicitMount, but got ShortMount($mount). The coursier module should use an ExplicitMount for clarity."
          )
      }
    }
  }
