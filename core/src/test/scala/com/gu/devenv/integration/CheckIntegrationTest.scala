...
class CheckIntegrationTest extends AnyFreeSpec with Matchers with TryValues {
  ...
  "check" - {
    ...
    "checking when no devcontainer files exist" - {
      "should return Mismatch for both files" in withTempDirs { (tempDir, userConfigDir) =>
        val devcontainerDir = tempDir.resolve(".devcontainer")
    ...
    "checking when files match configuration" - {
      "should return Match result" in withTempDirs { (tempDir, userConfigDir) =>
        val devcontainerDir = tempDir.resolve(".devcontainer")
    ...
    "checking when only user file is out of date" - {
      "should return Mismatch with only user diff" in withTempDirs { (tempDir, userConfigDir) =>
        val devcontainerDir = tempDir.resolve(".devcontainer")
    ...
    "checking when only shared file is out of date" - {
      "should return Mismatch with only shared diff" in withTempDirs { (tempDir, userConfigDir) =>
        val devcontainerDir = tempDir.resolve(".devcontainer")
    ...
    "checking when both files are out of date" - {
      "should return Mismatch with both diffs" in withTempDirs { (tempDir, userConfigDir) =>
        val devcontainerDir = tempDir.resolve(".devcontainer")
    ...
    "checking after config change" - {
      "should detect mismatch when config changes" in withTempDirs { (tempDir, userConfigDir) =>
        val devcontainerDir = tempDir.resolve(".devcontainer")
      ...
      "should show Match after regenerating" in withTempDirs { (tempDir, userConfigDir) =>
        val devcontainerDir = tempDir.resolve(".devcontainer")
    ...
    "checking with user config" - {
      "should match when user config is present" in withTempDirs { (tempDir, userConfigDir) =>
        val devcontainerDir = tempDir.resolve(".devcontainer")
