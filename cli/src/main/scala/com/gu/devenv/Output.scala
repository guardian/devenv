...
object Output {
  ...
  def usageMessage(
    ...
    s"""${formatter.sectionHeading("Usage:")} devenv <command> [--help]
       |
       ...
       |
       |${formatter.sectionHeading("Important:")} Run all commands from the root of your project.
  ...
  def versionMessage(
    ...
    s"${formatter.emphasis(release)}${Str(architectureStr)}${Str(branchStr)}"
  }
  ...
  def updateCheckResultMessage(
    ...
    result match {
      case Success(UpdateCheckResult.UpToDate) =>
        ...
        val message =
          formatter.success(
            s"Devenv ${formatter.emphasis(currentVersion)} is the latest version."
          )
        s"""$header
           |$divider
           |$message
      ...
      case Success(UpdateCheckResult.DevMode(latestRelease)) =>
        ...
        val latest =
          formatter.warning(
            s"The latest released version is ${formatter.emphasis(latestRelease.tagName)}"
          )
        s"""$header
           |$divider
           |$message
      ...
      case Success(UpdateCheckResult.NoCompatibleAsset(newerRelease)) =>
        ...
        val detail  = formatter.error(
          s"No compatible download was found for: ${formatter.emphasis(architecture.getOrElse("unknown"))}"
        )
        val release = formatter.link(newerRelease.htmlUrl)
        s"""$header
           |$divider
