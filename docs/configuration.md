...
- **`mise`** - Installs and configures [mise](https://mise.jdx.dev/) for version management of languages and tools.
  After its setup script succeeds, devenv evaluates `mise activate --shims bash` in the lifecycle
  shell, making installed tools available to subsequent modules, dotfiles, project commands and their
  child processes. Activation also runs before project `postStartCommand` commands, using the
  existing installation. Commands such as `npm install` need no `mise exec` wrapper.
  Mise determines its own paths at runtime, including any configured data directory; devenv does not
  modify PATH before installing mise. This does not affect `onCreateCommand`.
  Enabled by default.
