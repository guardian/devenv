...
Every module must declare one of the following `ModuleAdoption` values:

| Value          | Readiness            | Effect on new project configurations                         |
|----------------|----------------------|--------------------------------------------------------------|
| `Default`      | Production-ready     | Added as an active module                                    |
| `OptIn`        | Production-ready     | Added as a commented module marked disabled by default       |
| `Experimental` | Not production-ready | Added as a commented module marked experimental              |

Use `Default` only for modules that are broadly useful to all projects, such as `mise`. Use `OptIn`
for production-ready tools that projects select according to their needs, such as language support.
Use `Experimental` for modules that are not yet stable enough for widespread use.

The adoption value determines how the module appears in the `devenv.yaml` template generated by
`devenv init`:

```yaml
# Modules: Built-in functionality
# - default-tool: Set up the default tool
# - optional-tool: Set up the optional tool
# - experimental-tool: Set up the experimental tool (experimental)
...
Confirm that your new module appears according to its adoption value: `Default` modules are active,
`OptIn` modules are commented out with the `(disabled by default)` annotation, and `Experimental`
modules are commented out with the `(experimental)` annotation.
...
| Step | Task                                                                                                |
|------|-----------------------------------------------------------------------------------------------------|
| 1    | Create `core/src/main/scala/com/gu/devenv/modules/<name>.scala`; choose its `ModuleAdoption`        |
